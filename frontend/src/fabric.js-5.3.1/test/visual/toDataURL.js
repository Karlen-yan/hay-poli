(function() {
  ...
  function toDataURL5(fabricCanvas, callback) {
    fabricCanvas.loadFromJSON(canvasWithObjects, function() {
      const dataurl = fabricCanvas.toDataURL({multiplier: 0.3});
      callback(dataurl);
    });
  }
  ...
  function toDataURL6(fabricCanvas, callback) {
    ...
    fabricCanvas.loadFromJSON(canvasWithObjects, function() {
      const dataUrl = fabricCanvas.toDataURL({multiplier: 4});
      callback(dataUrl);
    });
  }
  ...
  function toDataURL7(fabricCanvas, callback) {
    ...
    fabricCanvas.setZoom(0.1);
    fabricCanvas.loadFromJSON(canvasWithObjects, function() {
      const dataUrl = fabricCanvas.toDataURL({multiplier: 12, left: 20, top: 20, width: 20, height: 20});
      callback(dataUrl);
    });
  }
  ...
  function toDataURL8(fabricCanvas, callback) {
    // make so everything is smaller
    fabricCanvas.loadFromJSON(canvasWithObjects, function() {
      const dataUrl = fabricCanvas.toDataURL({multiplier: 1.2, left: 200, top: 200, width: 200, height: 200});
      callback(dataUrl);
    });
  }
  ...
  function toDataURL9(fabricCanvas, callback) {
    ...
    fabricCanvas.loadFromJSON(canvasWithObjects, function() {
      const dataUrl = fabricCanvas.toDataURL({multiplier: 0.4, left: 600, top: 600, width: 600, height: 600});
      callback(dataUrl);
    });
  }
  ...
  function toDataURL10(fabricCanvas, callback) {
    ...
    const dataUrl = fabricCanvas.toDataURL({multiplier: 0.5});
    callback(dataUrl);
  }
  ...
  function toDataURL12(fabricCanvas, callback) {
    ...
    rect.shadow = new fabric.Shadow(shadow);
    fabricCanvas.add(rect);
    const dataUrl = fabricCanvas.toDataURL({multiplier: 0.5, enableRetinaScaling: true});
    fabric.devicePixelRatio = 1;
    callback(dataUrl);
  }
  ...
  function toDataURLWithOddPixels(fabricCanvas, callback) {
    ...
    const imageEl = fabric.util.createImage();
  ...
  function toDataURLWithOddPixelsStrokeWidth(fabricCanvas, callback) {
    ...
    const imageEl = fabric.util.createImage();
    imageEl.onload = function() {
      const fimg = new fabric.Image(imageEl, {strokeWidth: 1, stroke: 'orange'});
      callback(fimg.toDataURL());
    };
    imageEl.src = imgsrc;
  }
  ...
  function toDataURLWithOddPixelsStrokeWidthEven(fabricCanvas, callback) {
    ...
    const imageEl = fabric.util.createImage();
    imageEl.onload = function() {
      const fimg = new fabric.Image(imageEl, {strokeWidth: 2, stroke: 'orange'});
      callback(fimg.toDataURL());
    };
    imageEl.src = imgsrc;
  }
