(function() {
  ...
  QUnit.test('getSource', function(assert) {
    const oColor = new fabric.Color('ffffff');
  ...
  QUnit.test('toHexa', function(assert) {
    const oColor = new fabric.Color('ffffffff');
  ...
  QUnit.test('toGrayscale', function(assert) {
    const oColor = new fabric.Color('ff5555');
  ...
  QUnit.test('toBlackWhite', function(assert) {
    const oColor = new fabric.Color('333333');
  ...
  QUnit.test('fromRgb', function(assert) {
    ...
    const originalRgb = 'rgb(255,255,255)';
    const oColor = fabric.Color.fromRgb(originalRgb);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
  ...
  QUnit.test('fromRgb (percentage values)', function(assert) {
    assert.ok(typeof fabric.Color.fromRgb === 'function');
    const originalRgb = 'rgb(100%,100%,100%)';
    const oColor = fabric.Color.fromRgb(originalRgb);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
    assert.equal(oColor.toRgb(), 'rgb(255,255,255)');
  ...
  QUnit.test('fromRgba (uppercase)', function(assert) {
    ...
    const originalRgba = 'RGBA(255,255,255,0.5)';
    const oColor = fabric.Color.fromRgba(originalRgba);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
    assert.equal(oColor.toHex(), 'FFFFFF');
  ...
  QUnit.test('fromRgba (with missing 0)', function(assert) {
    const originalRgba = 'rgba( 255 , 255 , 255 , .3 )';
    const oColor = fabric.Color.fromRgba(originalRgba);
  ...
  QUnit.test('fromRgba (percentage values)', function(assert) {
    const originalRgba = 'rgba(100%,100%,100%,0.5)';
    const oColor = fabric.Color.fromRgba(originalRgba);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
    assert.equal(oColor.toRgba(), 'rgba(255,255,255,0.5)');
  ...
  QUnit.test('fromRgba (percentage values with decimals)', function(assert) {
    const originalRgba = 'rgba( 100.00%, 100.00%, 100.00% , 0.5 )';
    const oColor = fabric.Color.fromRgba(originalRgba);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
    assert.equal(oColor.toRgba(), 'rgba(255,255,255,0.5)');
  ...
  QUnit.test('fromHsl', function(assert) {
    ...
    const originalHsl = 'hsl(262,80%,12%)';
    const oColor = fabric.Color.fromHsl(originalHsl);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
  ...
  QUnit.test('fromHsl (uppercase)', function(assert) {
    assert.ok(typeof fabric.Color.fromHsl === 'function');
    const originalHsl = 'HSL(270,50%,40%)';
    const oColor = fabric.Color.fromHsl(originalHsl);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
  ...
  QUnit.test('fromHsla (uppercase)', function(assert) {
    ...
    const originalHsla = 'HSLA(108,50%,50%,0.7)';
    const oColor = fabric.Color.fromHsla(originalHsla);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
  ...
  QUnit.test('fromHsla', function(assert) {
    assert.ok(typeof fabric.Color.fromHsla === 'function');
    const originalHsla = 'hsla(262,80%,12%,0.2)';
    const oColor = fabric.Color.fromHsla(originalHsla);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
  ...
  QUnit.test('fromHex', function(assert) {
    ...
    const originalHex = 'FF5555';
    const oColor = fabric.Color.fromHex(originalHex);
    assert.ok(oColor);
    assert.ok(oColor instanceof fabric.Color);
  ...
  QUnit.test('fromSource', function(assert) {
    ...
    const oColor = fabric.Color.fromSource([255, 255, 255, 0.37]);
  ...
  QUnit.test('overlayWith', function(assert) {
    let oColor = new fabric.Color('FF0000');
