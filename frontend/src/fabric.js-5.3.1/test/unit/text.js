(function() {
  ...
  QUnit.test('get bounding rect after init', function(assert) {
    const string = 'Some long text, the quick brown fox jumps over the lazy dog etc... blah blah blah';
  ...
  QUnit.test('fabric.Text.fromElement', function(assert) {
    ...
    const elText = fabric.document.createElement('text');
    elText.textContent = 'x';

    fabric.Text.fromElement(elText, function(text) {
      assert.ok(text instanceof fabric.Text);
      const expectedObject = fabric.util.object.extend(fabric.util.object.clone(REFERENCE_TEXT_OBJECT), {
        left: 0,
        top: -14.05,
        width: 8,
        height: 18.08,
  ...
  QUnit.test('fabric.Text.fromElement with custom attributes', function(assert) {
    ...
    const elTextWithAttrs = fabric.document.createElementNS(namespace, 'text');
  ...
  QUnit.test('text styleHas', function(assert) {
    const text = new fabric.Text('xxxxxx\nx y');
    text.styles = { };
  ...
  QUnit.test('text cleanStyle with full style', function(assert) {
    const text = new fabric.Text('xxx');
  ...
  QUnit.test('getFontCache works with fontWeight numbers', function(assert) {
    const text = new fabric.Text('xxx', {fontWeight: 400});
    text.initDimensions();
    const cache = fabric.charWidthsCache[text.fontFamily.toLowerCase()];
  ...
  QUnit.test('getFontCache is case insensitive', function(assert) {
    const text = new fabric.Text('xxx', {fontWeight: 'BOld', fontStyle: 'NormaL'});
    text.initDimensions();
    const text2 = new fabric.Text('xxx', {fontWeight: 'bOLd', fontStyle: 'nORMAl'});
    text2.initDimensions();
    const cache = text.getFontCache(text);
    const cache2 = text2.getFontCache(text2);
    assert.equal(cache, cache2, 'you get the same cache');
  });
  // moved
  QUnit.test('getSelectionStyles with no arguments', function(assert) {
    const iText = new fabric.Text('test foo bar-baz\nqux', {
      styles: {
        0: {
  ...
  QUnit.test('_splitText', function(assert) {
    const text = new fabric.Text('test foo bar-baz\nqux', {});
    const test = text._splitText();
  ...
  QUnit.test('getSvgSpanStyles produces correct output', function(assert) {
    ...
    const expected = 'stroke-width: 30; font-family: \'Verdana\'; font-size: 25px; fill: rgb(255,0,0); ';
  ...
  QUnit.test('getSvgSpanStyles produces correct output with useWhiteSpace', function(assert) {
    ...
    const expected = 'stroke-width: 30; font-family: \'Verdana\'; font-size: 25px; fill: rgb(255,0,0); white-space: pre; ';
    assert.equal(styleString, expected, 'style is as expected');
  });
  ...
  QUnit.test('text superscript', function(assert) {
    const text = new fabric.Text('xxx', {styles: {
    ...
    const schema = text.superscript;
    const styleFontSize = text.styles[0][2].fontSize;
    const styleDeltaY = text.styles[0][2].deltaY;
  ...
  QUnit.test('_deleteStyleDeclaration', function(assert) {
    const text = new fabric.Text('aaa aaq ggg gg oee eee', {
      styles: {
        0: {
          0: {fontSize: 4},
          1: {fontSize: 4},
  ...
  QUnit.test('styleHas', function(assert) {
    const textbox = new fabric.Textbox('aaa\naaq ggg gg oee eee', {
      styles: {
        0: {
          0: {fontSize: 4},
          1: {fontSize: 4},
  ...
  QUnit.test('_getLineLeftOffset', function(assert) {
    const text = new fabric.Text('long line of text\nshort');
