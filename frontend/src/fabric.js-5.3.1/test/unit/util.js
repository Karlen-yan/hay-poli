(function() {
  ...
  const IMG_URL = fabric.isLikelyNode ?
  ...
  QUnit.test('fabric.util.toFixed', function(assert) {
    ...
    function test(what) {
      ...
      const fractionless = (typeof what === 'number') ?
        parseInt(what) :
  ...
  QUnit.test('fabric.util.getRandomInt', function(assert) {
    ...
    const randomInts = [];
    for (let i = 100; i--; ) {
      const randomInt = fabric.util.getRandomInt(100, 200);
      randomInts.push(randomInt);
      assert.ok(randomInt >= 100 && randomInt <= 200);
    }

    const areAllTheSame = randomInts.every(function(value) {
  ...
  QUnit.test('fabric.util.string.camelize', function(assert) {
    const camelize = fabric.util.string.camelize;
  ...
  QUnit.test('fabric.util.string.graphemeSplit', function(assert) {
    const gSplit = fabric.util.string.graphemeSplit;
  ...
  QUnit.test('fabric.util.string.escapeXml', function(assert) {
    const escapeXml = fabric.util.string.escapeXml;
  ...
  QUnit.test('fabric.util.string.capitalize', function(assert) {
    const capitalize = fabric.util.string.capitalize;
  ...
  QUnit.test('Function.prototype.bind', function(assert) {
    ...
    obj = { };
    const YAxisPoint = Point.bind(obj, 0);
  ...
  QUnit.test('fabric.util.getById', function(assert) {
    ...
    const el = fabric.document.createElement('div');
    el.id = 'foobarbaz';
  ...
  QUnit.test('fabric.util.toArray', function(assert) {
    ...
    const nodelist = fabric.document.getElementsByTagName('div'),
  ...
  QUnit.test('fabric.util.makeElement', function(assert) {
    const makeElement = fabric.util.makeElement;
  ...
  QUnit.test('fabric.util.addClass', function(assert) {
    const addClass = fabric.util.addClass;
  ...
  QUnit.test('fabric.util.wrapElement', function(assert) {
    const wrapElement = fabric.util.wrapElement;
    assert.ok(typeof wrapElement === 'function');

    let el = fabric.document.createElement('p');
    let wrapper = wrapElement(el, 'div');
    ...
    const childEl = fabric.document.createElement('span');
    const parentEl = fabric.document.createElement('p');
  ...
  QUnit.test('fabric.util.makeElementUnselectable', function(assert) {
    const makeElementUnselectable = fabric.util.makeElementUnselectable;

    assert.ok(typeof makeElementUnselectable === 'function');

    const el = fabric.document.createElement('p');
  ...
  QUnit.test('fabric.util.makeElementSelectable', function(assert) {
    const makeElementSelectable = fabric.util.makeElementSelectable,
  ...
  const SVG_DOC_AS_STRING = '<?xml version="1.0"?>\
  ...
  QUnit.test('fabric.util.loadImage', function(assert) {
    ...
    fabric.util.loadImage(IMG_URL, function(obj, isError) {
      if (obj) {
        const oImg = new fabric.Image(obj);
  ...
  const SVG_WITH_1_ELEMENT = '<?xml version="1.0"?>\
    <!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\
      <polygon fill="red" stroke="blue" stroke-width="10" points="350, 75 379,161 469,161\
        397,215 423,301 350,250 277,301 303,215 231,161 321,161" />\
  ...
  const SVG_WITH_2_ELEMENTS = '<?xml version="1.0"?>\
    <!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\
      <polygon fill="red" stroke="blue" stroke-width="10" points="350, 75 379,161 469,161\
        397,215 423,301 350,250 277,301 303,215 231,161 321,161" />\
  ...
  QUnit.test('fabric.util.createClass', function(assert) {
    const Klass = fabric.util.createClass();

    assert.ok(typeof Klass === 'function');
    assert.ok(typeof new Klass() === 'object');

    const Person = fabric.util.createClass({
    ...
    const john = new Person('John', 'Meadows');
    ...
    const WebDeveloper = fabric.util.createClass(Person, {
    ...
    const dan = new WebDeveloper('Dan', 'Trink', ['HTML', 'CSS', 'Javascript']);
  ...
  QUnit.test('fabric.util.array.invoke', function(assert) {
    ...
    const obj1 = {toString: function() {return 'obj1';}};
    const obj2 = {toString: function() {return 'obj2';}};
    const obj3 = {toString: function() {return 'obj3';}};
  ...
  QUnit.test('fabric.util.array.min', function(assert) {
    ...
    const obj1 = {valueOf: function() {return 1;}};
    const obj2 = {valueOf: function() {return 2;}};
    const obj3 = {valueOf: function() {return 3;}};
  ...
  QUnit.test('multiplyTransformMatrices', function(assert) {
    ...
    let m1 = [1, 1, 1, 1, 1, 1], m2 = [1, 1, 1, 1, 1, 1], m3;
  ...
  QUnit.test('saveObjectTransform', function(assert) {
    ...
    const transform = fabric.util.saveObjectTransform(rect);
  ...
  QUnit.test('rotatePoint', function(assert) {
    ...
    const origin = new fabric.Point(3, 0);
    const point = new fabric.Point(4, 0);
  ...
  QUnit.test('transformPoint', function(assert) {
    ...
    const point = new fabric.Point(2, 2);
    const matrix = [3, 0, 0, 2, 10, 4];
    const tp = fabric.util.transformPoint(point, matrix);
  ...
  QUnit.test('createImage', function(assert) {
    ...
    const element = fabric.util.createImage();
  ...
  QUnit.test('qrDecompose with identity matrix', function(assert) {
    ...
    const options = fabric.util.qrDecompose(fabric.iMatrix);
  ...
  QUnit.test('qrDecompose with matrix', function(assert) {
    assert.ok(typeof fabric.util.qrDecompose === 'function');
    const options = fabric.util.qrDecompose([2, 0.4, 0.5, 3, 100, 200]);
  ...
  QUnit.test('fabric.util.capValue ar < 1', function(assert) {
    ...
    const val = fabric.util.capValue(3, 10, 70);
    assert.equal(val, 10, 'value is not capped');
  });

  QUnit.test('fabric.util.capValue ar < 1', function(assert) {
    assert.ok(typeof fabric.util.capValue === 'function');
    const val = fabric.util.capValue(3, 1, 70);
    assert.equal(val, 3, 'min cap');
  });

  QUnit.test('fabric.util.capValue ar < 1', function(assert) {
    assert.ok(typeof fabric.util.capValue === 'function');
    const val = fabric.util.capValue(3, 80, 70);
  ...
  QUnit.test('fabric.util.copyCanvasElement', function(assert) {
    ...
    const b = fabric.util.copyCanvasElement(c);
  ...
  QUnit.test('fabric.util.transformPath can scale a path by 2', function(assert) {
    ...
    const path = new fabric.Path('M 100 100 L 200 100 L 170 200 z');
    const oldPath = path.path;
    const newPath = fabric.util.transformPath(path.path, [2, 0, 0, 2, 0, 0]);
  ...
  QUnit.test('fabric.util.transformPath can apply a generic transform', function(assert) {
    const path = new fabric.Path('M 100 100 L 200 100 L 170 200 z');
    const oldPath = path.path;
    const newPath = fabric.util.transformPath(path.path, [1, 2, 3, 4, 5, 6], path.pathOffset);
    assert.equal(fabric.util.joinPath(oldPath), 'M 100 100 L 200 100 L 170 200 z');
  ...
  QUnit.test('fabric.util.calcDimensionsMatrix', function(assert) {
    ...
    let matrix = fabric.util.calcDimensionsMatrix({
      scaleX: 2,
      scaleY: 3,
      skewY: 10,
      skewX: 5,
