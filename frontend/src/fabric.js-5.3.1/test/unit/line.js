(function() {
  ...
  QUnit.test('constructor', function(assert) {
    ...
    const line = new fabric.Line([10, 11, 20, 21]);
    ...
    const lineWithoutPoints = new fabric.Line();
  ...
  QUnit.test('fromElement', function(assert) {
    ...
    const lineEl = fabric.document.createElementNS(namespace, 'line'),
        x1 = 11,
        y1 = 23,
        x2 = 34,
        y2 = 7,
    ...
    fabric.Line.fromElement(lineEl, function(oLine) {
      ...
      const lineElWithMissingAttributes = fabric.document.createElementNS(namespace, 'line');
  ...
  QUnit.test('straight lines may have 0 width or height', function(assert) {
    const line1 = new fabric.Line([10, 10, 100, 10]),
  ...
  lineCoordsCases.forEach(function(c_) {
    QUnit.test('stroke-less line coords ' + c_.description, function(assert) {
      const points = c_.givenLineArgs.points;
      const options = c_.givenLineArgs.options;

      const givenLine = new fabric.Line(
        points,
        options,
      );
