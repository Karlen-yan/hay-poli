(function() {
  ...
  QUnit.test('_beforeTransform', function(assert) {
    ...
    const rect = new fabric.Rect({left: 50, top: 50, width: 50, height: 50});
    canvas.add(rect);
    ...
    const corners = ['tl', 'mt', 'tr', 'mr', 'br', 'mb', 'bl', 'ml', 'mtr'];
  ...
  QUnit.test('cache and reset event properties', function(assert) {
    const e = {clientX: 30, clientY: 30, which: 1, target: canvas.upperCanvasEl};
    const rect = new fabric.Rect({width: 60, height: 60});
  ...
  QUnit.test('mouse:down and group selector', function(assert) {
    ...
    const expectedGroupSelector = {ex: 80, ey: 120, top: 0, left: 0};
  ...
  QUnit.test('activeOn object selection', function(assert) {
    const rect = new fabric.Rect({width: 200, height: 200, activeOn: 'down'});
    canvas.add(rect);
    const e = {clientX: 30, clientY: 15, which: 1, target: canvas.upperCanvasEl};
    canvas.__onMouseDown(e);
  ...
  QUnit.test('specific bug #5317 for shift+click and active selection', function(assert) {
    const greenRect = new fabric.Rect({
      width: 300,
      height: 300,
      left: 0,
      top: 0,
    ...
    const e = {clientX: 40, clientY: 40, which: 1, target: canvas.upperCanvasEl};
    canvas.__onMouseDown(e);
    assert.equal(canvas._activeObject, blueCircle, 'blue circle is selected with first click');
    canvas.__onMouseUp(e);
    const e2 = {clientX: 240, clientY: 140, which: 1, target: canvas.upperCanvasEl, shiftKey: true};
    ...
    const e3 = {clientX: 140, clientY: 90, which: 1, target: canvas.upperCanvasEl, shiftKey: true};
    ...
    const e4 = {clientX: 290, clientY: 290, which: 1, target: canvas.upperCanvasEl};
  ...
  QUnit.test('setDimensions and active brush', function(assert) {
    ...
    let rendered = false;
    const canva = new fabric.Canvas(null, {width: 500, height: 500});
    const brush = new fabric.PencilBrush({color: 'red', width: 4});
  ...
  QUnit.test('mouse:up isClick = false', function(assert) {
    const e = {clientX: 30, clientY: 30, which: 1};
    const e2 = {clientX: 31, clientY: 31, which: 1};
    let isClick = true;
    canvas.on('mouse:up', function(opt) {
      isClick = opt.isClick;
    });
  ...
  QUnit.test('mouse:up should return target and currentTarget', function(assert) {
    const e1 = {clientX: 30, clientY: 30, which: 1};
    const e2 = {clientX: 100, clientY: 100, which: 1};
    const rect1 = new fabric.Rect({left: 0, top: 0, width: 50, height: 50, lockMovementX: true, lockMovementY: true});
    const rect2 = new fabric.Rect({left: 75, top: 75, width: 50, height: 50});
    canvas.add(rect1);
    canvas.add(rect2);
    let opt;
  ...
  QUnit.test('drag small object when mousemove + drag, not active', function(assert) {
    const e = {clientX: 2, clientY: 2, which: 1};
    const e1 = {clientX: 4, clientY: 4, which: 1};
    const e2 = {clientX: 6, clientY: 6, which: 1};
    const rect = new fabric.Rect({left: 0, top: 0, width: 3, height: 3, strokeWidth: 0});
    canvas.add(rect);
    canvas.__onMouseDown(e);
  ...
  QUnit.test('scale small object when mousemove + drag, active', function(assert) {
    const e = {clientX: 3, clientY: 3, which: 1};
    const e1 = {clientX: 6, clientY: 6, which: 1};
    const e2 = {clientX: 9, clientY: 9, which: 1};
    const rect = new fabric.Rect({left: 0, top: 0, width: 3, height: 3, strokeWidth: 0});
  ...
  QUnit.test('A transform than ends outside the object will call mouseup handler', function(assert) {
    ...
    const e1 = {clientX: 6, clientY: 6, which: 1};
    const e2 = {clientX: 9, clientY: 9, which: 1};
    const e3 = {clientX: 100, clientY: 100, which: 1};
    const rect = new fabric.Rect({left: 0, top: 0, width: 3, height: 3, strokeWidth: 0});
    let mouseUpCalled = false;
    rect.controls = {
      br: fabric.Object.prototype.controls.br,
  ...
  QUnit.test('A transform than ends on a new control, calls both mouseup handler', function(assert) {
    ...
    const e1 = {clientX: 6, clientY: 6, which: 1};
    const e2 = {clientX: 9, clientY: 9, which: 1};
    const e3 = {clientX: 9, clientY: 3, which: 1};
    const rect = new fabric.Rect({left: 0, top: 0, width: 3, height: 3, strokeWidth: 0});
  ...
  ['DragEnter', 'DragLeave', 'DragOver', 'Drop'].forEach(function(eventType) {
    QUnit.test('avoid multiple registration - ' + eventType, function(assert) {
      ...
      c.initialize(c.lowerCanvasEl);
      const event = fabric.document.createEvent('HTMLEvents');
  ...
  QUnit.test('Fabric event fired - Drop', function(assert) {
    const eventNames = ['drop:before', 'drop'];
    const c = new fabric.Canvas();
    const fired = [];
  ...
  ['mousedown', 'mousemove', 'wheel', 'dblclick'].forEach(function(eventType) {
    QUnit.test('Fabric event fired - ' + eventType, function(assert) {
      let eventname = eventType.slice(0, 5) + ':' + eventType.slice(5);
      ...
      let counter = 0;
      const c = new fabric.Canvas();
      const rect = new fabric.Rect({top: -4, left: -4, width: 12, height: 12});
      c.add(rect);
      c.on(eventname, function(opt) {
        counter++;
        target = opt.target;
  ...
  QUnit.test('mouseover and mouseout with subtarget check', function(assert) {
    const rect1 = new fabric.Rect({width: 5, height: 5, left: 5, top: 0, strokeWidth: 0, name: 'rect1'});
    const rect2 = new fabric.Rect({width: 5, height: 5, left: 5, top: 5, strokeWidth: 0, name: 'rect2'});
    const rect3 = new fabric.Rect({width: 5, height: 5, left: 0, top: 5, strokeWidth: 0, name: 'rect3'});
    const rect4 = new fabric.Rect({width: 5, height: 5, left: 0, top: 0, strokeWidth: 0, name: 'rect4'});
    const rect5 = new fabric.Rect({width: 5, height: 5, left: 2.5, top: 2.5, strokeWidth: 0, name: 'rect5'});
    const group1 = new fabric.Group([rect1, rect2], {subTargetCheck: true, name: 'group1'});
    const group2 = new fabric.Group([rect3, rect4], {subTargetCheck: true, name: 'group2'});
    // a group with 2 groups, with 2 rects each, one group left one group right
    // each with 2 rects vertically aligned
    const group = new fabric.Group([group1, group2], {subTargetCheck: true, name: 'group'});
    const c = new fabric.Canvas();
    let targetArray = [];
    let targetOutArray = [];
  ...
  QUnit.test('Fabric mouseover, mouseout events fire for subTargets when subTargetCheck is enabled', function(assert) {
    let counterOver = 0, counterOut = 0, canvas = new fabric.Canvas();
    ...
    canvas.loadFromJSON(SUB_TARGETS_JSON, function() {
      ...
      const moveEvent = fabric.document.createEvent('HTMLEvents');
      moveEvent.initEvent('mousemove', true, true);
      const target = canvas.item(1);
      canvas.targets = [
        target.item(1),
  ...
  QUnit.test('mouse move and group selector', function(assert) {
    const e = {clientX: 30, clientY: 40, which: 1, target: canvas.upperCanvasEl};
    const expectedGroupSelector = {ex: 15, ey: 30, left: 65, top: 90};
    canvas.absolutePan(new fabric.Point(50, 80));
  ...
  ['MouseDown', 'MouseMove', 'MouseOut', 'MouseEnter', 'MouseWheel', 'DoubleClick'].forEach(function(eventType) {
    QUnit.test('avoid multiple registration - ' + eventType, function(assert) {
      const funcName = '_on' + eventType;
      let eventName = eventType.toLowerCase();
      ...
      c.initialize(c.lowerCanvasEl);
      c.initialize(c.lowerCanvasEl);
      const event = fabric.document.createEvent('MouseEvent');
      event.initEvent(eventName, true, true);
      c.upperCanvasEl.dispatchEvent(event);
      assert.equal(counter, 1, eventName + ' listener executed once');
    });
  ...
  QUnit.test('avoid multiple registration - mouseup', function(assert) {
    ...
    const originalMouseUp = fabric.Canvas.prototype._onMouseUp;
    let counter = 0;
    ...
    const _event = fabric.document.createEvent('MouseEvent');
  ...
  QUnit.test('mouseEnter removes __corner', function(assert) {
    ...
    event.initEvent('mouseenter', true, true);
    const c = new fabric.Canvas();
    const obj = new fabric.Object({top: 100, left: 100});
    c.add(obj);
  ...
  QUnit.test('avoid multiple events on window', function(assert) {
    const originalResize = fabric.Canvas.prototype._onResize;
    let counter = 0;
    ...
    c.initialize(c.lowerCanvasEl);
    c.initialize(c.lowerCanvasEl);
    const event = fabric.document.createEvent('UIEvents');
  ...
  QUnit.test('getCornerCursor ', function(assert) {
    ...
    let target = new fabric.Object({canvas: canvas});
    var e = { };
    e[key] = false;
