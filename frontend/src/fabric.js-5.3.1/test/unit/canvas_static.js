(function() {
  ...
  const CANVAS_SVG = '<?xml version="1.0" encoding="UTF-8" standalone="no" ?>\n<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n' +
                   '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="200" height="200" viewBox="0 0 200 200" xml:space="preserve">\n<desc>Created with Fabric.js ' + fabric.version + '</desc>\n<defs>\n</defs>\n</svg>';

  const CANVAS_SVG_VIEWBOX = '<?xml version="1.0" encoding="UTF-8" standalone="no" ?>\n<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n' +
  ...
  const PATH_JSON = '{"version":"' + fabric.version + '","objects": [{"type": "path", "version":"' + fabric.version + '", "originX": "left", "originY": "top", "left": 268, "top": 266, "width": 51, "height": 49,' +
  ...
  const PATH_WITHOUT_DEFAULTS_JSON = '{"version":"' + fabric.version + '","objects": [{"type": "path", "version":"' + fabric.version + '", "left": 268, "top": 266, "width": 51, "height": 49, "path": [["M", 18.511, 13.99],' +
                  ' ["c", 0, 0, -2.269, -4.487, -12.643, 4.411], ["c", 0, 0, 4.824, -14.161, 19.222, -9.059],' +
                  ' ["l", 0.379, -2.1], ["c", -0.759, -0.405, -1.375, -1.139, -1.645, -2.117], ["c", -0.531, ' +
                  '-1.864, 0.371, -3.854, 1.999, -4.453], ["c", 0.312, -0.118, 0.633, -0.169, 0.953, -0.169], ' +
                  '["c", 1.299, 0, 2.514, 0.953, 2.936, 2.455], ["c", 0.522, 1.864, -0.372, 3.854, -1.999, ' +
  ...
  const RECT_JSON_WITH_PADDING = '{"version":"' + fabric.version + '","objects":[{"type":"rect","version":"' + fabric.version + '","originX":"left","originY":"top","left":0,"top":0,"width":10,"height":20,"fill":"rgb(0,0,0)",' +
                               '"stroke":null,"strokeWidth":1,"strokeDashArray":null,"strokeLineCap":"butt","strokeDashOffset":0,"strokeLineJoin":"miter","strokeUniform":false,"strokeMiterLimit":4,' +
                               '"scaleX":1,"scaleY":1,"angle":0,"flipX":false,"flipY":false,"opacity":1,' +
  ...
  // TODO: fix this
  const canvas = this.canvas = new fabric.StaticCanvas(null, {renderOnAddRemove: false, enableRetinaScaling: false, width: 200, height: 200});
  const canvas2 = this.canvas2 = new fabric.StaticCanvas(null, {renderOnAddRemove: false, enableRetinaScaling: false, width: 200, height: 200});
  ...
  QUnit.test('getObjects with type', function(assert) {
    const rect = new fabric.Rect({width: 10, height: 20});
    const circle = new fabric.Circle({radius: 30});
  ...
  QUnit.test('clear', function(assert) {
    ...
    const bg = new fabric.Rect({width: 10, height: 20});
  ...
  QUnit.test('toDataURL', function(assert) {
    ...
    const rect = new fabric.Rect({width: 100, height: 100, fill: 'red', top: 0, left: 0});
    canvas.add(rect);
  ...
  QUnit.test('toDataURL with enableRetinaScaling: true and no multiplier', function(assert) {
    ...
    const c = new fabric.StaticCanvas(null, {enableRetinaScaling: true, width: 10, height: 10});
  ...
  QUnit.test('toDataURL with enableRetinaScaling: false and multiplier = 1', function(assert) {
    ...
    fabric.devicePixelRatio = 2;
    const c = new fabric.StaticCanvas(null, {enableRetinaScaling: true, width: 10, height: 10});
    const dataUrl = c.toDataURL({enableRetinaScaling: false, multiplier: 1});
    c.cancelRequestedRender();
    const img = fabric.document.createElement('img');
    img.onload = function() {
      assert.equal(img.width, c.width, 'output width is not bigger');
  ...
  QUnit.test('toDataURL with enableRetinaScaling: false and multiplier = 3', function(assert) {
    ...
    fabric.devicePixelRatio = 2;
    const c = new fabric.StaticCanvas(null, {enableRetinaScaling: true, width: 10, height: 10});
    const dataUrl = c.toDataURL({enableRetinaScaling: false, multiplier: 3});
    c.cancelRequestedRender();
    const img = fabric.document.createElement('img');
    img.onload = function() {
  ...
  QUnit.test('viewportCenterObjectH', function(assert) {
    ...
    const rect = makeRect({left: 102, top: 202}), pan = 10;
    canvas.viewportTransform = [1, 0, 0, 1, 0, 0];
    canvas.add(rect);
    const oldY = rect.top;
  ...
  QUnit.test('toSVG with viewBox', function(assert) {
    ...
    const svg = canvas.toSVG({viewBox: {x: 100, y: 100, width: 300, height: 300}});
  ...
  QUnit.test('toSVG with exclude from export', function(assert) {
    ...
    const circle = new fabric.Circle({excludeFromExport: true}),
  ...
  QUnit.test('toSVG with a clipPath', function(assert) {
    const canvasClip = new fabric.StaticCanvas(null, {width: 400, height: 400, renderOnAddRemove: false});
  ...
  QUnit.test('toSVG with exclude from export background', function(assert) {
    const image = fabric.document.createElement('img'),
    ...
    const expectedSVG2 = '<?xml version="1.0" encoding="UTF-8" standalone="no" ?>\n<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="200" height="200" viewBox="0 0 200 200" xml:space="preserve">\n<desc>Created with Fabric.js ' + fabric.version + '</desc>\n<defs>\n</defs>\n</svg>';
  ...
  QUnit.test('toJSON custom properties non-existence check', function(assert) {
    ...
    const data = canvas.toJSON(['padding', 'foo', 'bar', 'baz']);
  ...
  QUnit.test('toJSON backgroundImage with custom props', function(assert) {
    ...
    createImageObject(function(image) {
      canvas.backgroundImage = image;
      image.custom = 'yes';
      const json = canvas.toJSON(['custom']);
  ...
  QUnit.test('toObject non includeDefaultValues', function(assert) {
    ...
    const expectedRect = {version: fabric.version, type: 'rect', width: 10, height: 10, top: 0, left: 0};
  ...
  QUnit.test('toObject excludeFromExport', function(assert) {
    const rect = makeRect(), rect2 = makeRect(), rect3 = makeRect(), clipPath = makeRect();
    canvas.clear();
  ...
  QUnit.test('loadFromJSON custom properties', function(assert) {
    ...
    const jsonWithoutFoo = JSON.stringify(canvas.toJSON(['padding']));
    const jsonWithFoo = JSON.stringify(canvas.toJSON(['padding', 'foo']));
  ...
  QUnit.test('setViewportTransform calls objects setCoords', function(assert) {
    const vpt = [2, 0, 0, 2, 50, 50];
    assert.deepEqual(canvas.viewportTransform, [1, 0, 0, 1, 0, 0], 'initial viewport is identity matrix');
    const rect = new fabric.Rect({width: 10, heigth: 10});
    const rectBg = new fabric.Rect({width: 10, heigth: 10});
    const rectOverlay = new fabric.Rect({width: 10, heigth: 10});
    canvas.add(rect);
    canvas.cancelRequestedRender();
  ...
  QUnit.test('zoomToPoint', function(assert) {
    ...
    assert.deepEqual(canvas.viewportTransform, [1, 0, 0, 1, 0, 0], 'initial viewport is identity matrix');
    const point = new fabric.Point(50, 50);
  ...
  QUnit.test('toSVG with background', function(assert) {
    const canvas2 = new fabric.StaticCanvas();
