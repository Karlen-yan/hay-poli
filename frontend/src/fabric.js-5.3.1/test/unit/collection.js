(function() {
  ...
  const collection2 = fabric.util.object.clone(collection);
  ...
  QUnit.test('add', function(assert) {
    ...
    const returned = collection.add(obj);
  ...
  QUnit.test('insertAt', function(assert) {
    ...
    const newObject = {prop: 5};
    previousObject = collection._objects[index];
    const returned = collection.insertAt(newObject, index, nonSplicing);
    assert.equal(returned, collection, 'is chainable');
  ...
  QUnit.test('remove', function(assert) {
    let obj = {prop: 4}, obj2 = {prop: 2}, obj3 = {prop: 3}, fired = 0;
  ...
  QUnit.test('forEachObject', function(assert) {
    ...
    const callback = function(_obj) {
      _obj.prop = true;
      fired++;
    };
    const returned = collection.forEachObject(callback);
    assert.equal(returned, collection, 'is chainable');
  ...
  QUnit.test('item', function(assert) {
    const obj = {type: 'a'}, obj2 = {type: 'b'}, index = 1;
    collection.add(obj2, obj);
  ...
  QUnit.test('isEmpty', function(assert) {
    ...
    let returned = collection.isEmpty();
  ...
  QUnit.test('contains', function(assert) {
    ...
    let returned = collection.contains(obj);
  ...
  QUnit.test('complexity', function(assert) {
    ...
    let returned = collection.complexity();
    assert.ok(typeof returned === 'number', 'returns a number');
    ...
    const complexObject = {complexity: function() {return 9;}};
    const complexObject2 = {complexity: function() {return 10;}};
