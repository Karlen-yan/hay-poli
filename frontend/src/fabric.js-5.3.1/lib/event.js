...
(function(root) {
  ...
  root.getEventSupport = function(target, type) {
    ...
    if (target.setAttribute && target.removeAttribute) {
      ...
      const isSupported = typeof target[type] === 'function';
  ...
  var eventManager = function(target, type, listener, configure, trigger, fromOverwrite) {
    ...
    if (typeof (target) === 'string' && type === 'ready') {
      if (window.eventjs_stallOnReady) { // / force stall for scripts to load
        ...
        const ms = configure.interval || 1000 / 60;
    ...
    if (target.length > 0 && target !== window) {
      for (let n0 = 0, length0 = target.length; n0 < length0; n0++) {
    ...
    if (typeof (type) !== 'string') { // Has multiple events.
      if (typeof (type.length) === 'number') { // Handle multiple listeners glued together.
        for (let n1 = 0, length1 = type.length; n1 < length1; n1++) { // Array [type]
    ...
    const useCapture = configure.useCapture || false;
    let id = getID(target) + '.' + getID(listener) + '.' + (useCapture ? 1 : 0);
    // Handle the event.
    if (root.Gesture && root.Gesture._gestureHandlers[type]) { // Fire custom event.
      ...
      if (trigger === 'remove') { // Remove event listener.
        ...
        if (configure.useCall && !root.modifyEventListener) {
          ...
          listener = function(event, self) {
            for (const key in self) {event[key] = self[key];}
      ...
      const eventList = getEventList(type);
  ...
  const getEventSupport = (function() {
    root.supports = {};
  ...
  var getEventList = (function() {
    return function(type) {
      const prefix = document.addEventListener ? '' : 'on'; // IE
      const idx = pointerDetect[type];
      if (isFinite(idx)) {
        const types = [];
        for (const key in root.supports) {
...
*/
  ...
  root.createPointerEvent = function(event, self, preventRecord) {
    const eventName = self.gesture;
    const target = self.target;
    const pts = event.changedTouches || root.proxy.getCoords(event);
    if (pts.length) {
      const pt = pts[0];
  ...
  var augmentEventListeners = function() {
    ...
    const augmentEventListener = function(proto) {
      const recall = function(trigger) { // overwrite native *EventListener's
        const handle = trigger + 'EventListener';
        const handler = proto[handle];
        proto[handle] = function(type, listener, useCapture) {
          if (root.Gesture && root.Gesture._gestureHandlers[type]) { // capture custom events.
            let configure = useCapture;
  ...
  var augmentSelectors = function() {
    ...
    const proto = NodeList.prototype;
    proto.removeEventListener = function(type, listener, useCapture) {
      for (let n = 0, length = this.length; n < length; n++) {
...
*/
  ...
  root.pointerSetup = function(conf, self) {
    ...
    let fingers = 0;
    const type = self.gesture.indexOf('pointer') === 0 && eventjs.modifyEventListener ? 'pointer' : 'mouse';
...
*/
  ...
  const sp = eventjs.supports; // Default pointerType
  // /
  ...
  root.pointerStart = function(event, self, conf) {
    ...
    const type = (event.type || 'mousedown').toUpperCase();
    ...
    // /
    const addTouchStart = function(touch, sid) {
      ...
      // /
      const x = touch.pageX - pt.offsetX;
      const y = touch.pageY - pt.offsetY;
      // /
      pt.rotation = 0;
      pt.scale = 1;
    ...
    // /
    const isTouchStart = !conf.fingers;
    var track = conf.tracker;
    const touches = event.changedTouches || root.getCoords(event);
...
*/
  ...
  const getPropertyAsFloat = function(o, type) {
    const n = parseFloat(o.getPropertyValue(type), 10);
  ...
  root.getBoundingBox = function(o) {
    ...
    const borderBox = style.getPropertyValue('box-sizing') === 'border-box';
    // /
    if (borderBox === false) {
      const left = getPropertyAsFloat(style, 'border-left-width');
      const right = getPropertyAsFloat(style, 'border-right-width');
      const bottom = getPropertyAsFloat(style, 'border-bottom-width');
      const top = getPropertyAsFloat(style, 'border-top-width');
    ...
    let tmp = position === 'fixed' ? o : o.parentNode;
    while (tmp !== null) {
...
*/
  ...
  (function() {
    ...
    const mac = agent.indexOf('macintosh') !== -1;
    let metaKeys;
...
eventjs.MutationObserver = (function() {
  ...
  const DOMAttrModifiedSupported = !MutationObserver && (function() {
    const p = document.createElement('p');
    let flag = false;
    const fn = function() {flag = true;};
  ...
  root.click = function(conf) {
    ...
    conf.onPointerUp = function(event) {
      if (root.pointerEnd(event, self, conf)) {
        ...
        const pointers = event.changedTouches || root.getCoords(event);
        const pointer = pointers[0];
        const bbox = conf.bbox;
        const newbbox = root.getBoundingBox(conf.target);
        const y = pointer.pageY - newbbox.scrollBodyTop;
        const x = pointer.pageX - newbbox.scrollBodyLeft;
        // //
...
root.dblclick = function(conf) {
  ...
  const delay = 700; // in milliseconds
  let time0, time1, timeout;
  let pointer0, pointer1;
  // Tracking the events.
  conf.onPointerDown = function(event) {
    const pointers = event.changedTouches || root.getCoords(event);
  ...
  conf.onPointerMove = function(event) {
    ...
    const bbox = conf.bbox;
    const ax = (pointer1.pageX - bbox.x1);
    const ay = (pointer1.pageY - bbox.y1);
...
eventjs.proxy = (function(root) {
  ...
  root.drag = function(conf) {
    ...
    conf.onPointerMove = function(event, state) {
      ...
      for (let i = 0; i < length; i++) {
        const touch = touches[i];
        const identifier = touch.identifier || Infinity;
        const pt = conf.tracker[identifier];
...
eventjs.proxy = (function(root) {
  ...
  const getCentroid = function(self, points) {
    let centroidx = 0;
    let centroidy = 0;
    let length = 0;
    for (const sid in points) {
      const touch = points[sid];
  ...
  root.gesture = function(conf) {
    ...
    conf.onPointerDown = function(event) {
      ...
      if (conf.fingers === conf.minFingers && fingers !== conf.fingers) {
        ...
        self.rotation = 0;
        self.state = 'start';
        let sids = ''; // - FIXME(mud): can generate duplicate IDs.
        for (const key in conf.tracker) {sids += key;}
    ...
    conf.onPointerMove = function(event, state) {
      ...
      for (var sid in points) {
        ...
        const rotate = (start.angle - angle + 360) % 360 - 180;
...
eventjs.proxy = (function(root) {
  ...
  root.shake = function(conf) {
    ...
    // Setting up local variables.
    const threshold = 4; // Gravitational threshold.
    const timeout = 1000; // Timeout between shake events.
    const timeframe = 200; // Time between shakes.
    const shakes = 3; // Minimum shakes to trigger event.
    ...
    var onDeviceMotion = function(e) {
      const alpha = 0.8; // Low pass filter.
      ...
      for (let n = 0, length = data.length; n < length; n++) {
        const letter = data[n];
        const ACCELERATION = self.acceleration[letter];
        const DELTA = delta[letter];
        const abs = Math.abs(ACCELERATION);
        // / Check whether another shake event was recently registered.
        if (now - lastShake < timeout) {continue;}
        // / Check whether delta surpasses threshold.
        if (abs > threshold) {
          const idx = now * ACCELERATION / abs;
          const span = Math.abs(idx + DELTA.value);
...
eventjs.proxy = (function(root) {
  ...
  root.swipe = function(conf) {
    ...
    conf.onPointerMove = function(event) {
      ...
      for (let i = 0; i < length; i++) {
        const touch = touches[i];
        const sid = touch.identifier || Infinity;
        const o = conf.tracker[sid];
        // Identifier defined outside of listener.
        if (!o) {continue;}
    ...
    conf.onPointerUp = function(event) {
      if (root.pointerEnd(event, self, conf)) {
        ...
        let length = 0;
        // /
        for (const sid in conf.tracker) {
          const touch = conf.tracker[sid];
          const xdist = touch.move.x - touch.start.x;
          const ydist = touch.move.y - touch.start.y;
          // /
          endx += touch.move.x;
          endy += touch.move.y;
          ...
          const ms = touch.moveTime - touch.startTime;
        ...
        // /
        const fingers = conf.gestureFingers;
  ...
  root.tap = function(conf) {
    ...
    conf.onPointerMove = function(event) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        const pt = conf.tracker[identifier];
        if (!pt) {continue;}
        const x = (touch.pageX - bbox.x1 - parseInt(window.scrollX));
        const y = (touch.pageY - bbox.y1 - parseInt(window.scrollY));
        // /
        const dx = x - pt.start.x;
        const dy = y - pt.start.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
...
eventjs.proxy = (function(root) {
  ...
  root.wheel = function(conf) {
    ...
    let interval;
    const timeout = conf.timeout || 150;
    let count = 0;
    // Externally accessible data.
    const self = {
      gesture: 'wheel',
      ...
      preventElasticBounce: function(event) {
        ...
        const top = scrollTop + target.offsetHeight;
